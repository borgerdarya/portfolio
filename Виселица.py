...
def play(word):
    ...
    print('Угадай слово:', word_completion, len(word), 'букв')
    while tries > 0:
        guess = input('Попробуй угадать букву или слово целиком \n').upper()
        if not guess.isalpha():
            print('Слово состоит из букв, а не других знаков.')
            continue
        elif guess in guessed_letters or guess in guessed_words:
            print('Ты уже называл эту букву/это слово, попробуй еще раз')
            continue
        else:
            if len(guess) == 1:
                ...
                if guess in word:
                    ...
                    print('Ты угадал букву:', word_completion)
                else:
                    print('В слове нет такой буквы')
                    tries -= 1
                    print('У тебя остались еще попытки:', tries)
                    print(display_hangman(tries))
                    continue
                ...
                if guess == word:
                    ...
                    print(display_hangman(tries))
                    continue
    return 'Попытки закончились, загаданное слово -', word

again = 'д'
while again == 'д':
    word = get_word(word_list)
    print(play(word))
    again = input('Хочешь сыграть еще? д - да, н - нет \n')
